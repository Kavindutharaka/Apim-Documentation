...
### Common Changes
- The default token binding type of both the Console and MyAccount is changed from ```SSO-based binding``` to ```cookie-based binding```. Read more on [Access Token Binding Type](https://is.docs.wso2.com/en/6.0.0/learn/configuring-oauth2-openid-connect-single-sign-on/).
...
For the same scenario in IS versions 5.11.0 and below, all the claims sent from the external IDP will be sent to the application regardless of the requested claim configuration. If the previous behavior is required in IS 6.0.0, add the following configuration to the ```deployment.toml``` file.
...
### Deleting locally added claims of JIT provisioned users

In Identity Server 5.11.0, with the `Assert identity using mapped local subject identifier` option enabled for a service provider, the locally added claim values for a JIT provisioned user during initial provisioning will be retained, even if those specific claim values are not returned by the federated Identity Provider (IdP) during subsequent federated logins.

In Identity Server 6.0.0, this behavior has been changed. The attribute synchronization process for a JIT provisioned user was changed so as to delete the existing claim mappings which are not present in the IDP claim mapping. This deletes existing local claim values that are not present in the subsequent federated logins after the initial provisioning.

If the previous behavior is required(To preserve locally added claims of JIT provisioned users), add the following configuration to the `deployment.toml` file.
...
### ID Token Signing Behavior (when Encryption is Used)
...
```
Read more about [Intermediate Certificate Validation](https://is.docs.wso2.com/en/6.0.0/apis/overview/#configure-intermediate-certificate-validation).
...
Read more on [User's Session Management API Definition](https://is.docs.wso2.com/en/6.0.0/apis/session-mgt-rest-api/).
...
1. Get the `migration.sh` script provided by the WSO2 team and run it using the command `sh migration.sh`.
...
In previous versions of WSO2 Identity Server, the default HTTP methods allowed for CORS were only ```GET```, ```POST```, ```HEAD``` and ```OPTIONS```.
...
Learn more on [how to change the CORS configuration](https://is.docs.wso2.com/en/6.0.0/deploy/configure-cors/).

### Log4j2 logging in Hazelcast
If you have been using WSO2 Identity Server in a [Hazelcast cluster](https://is.docs.wso2.com/en/6.0.0/deploy/configure-hazelcast/), you may have configured the logging type for Hazelcast as log4j ```(Log4j1)```. Log4j1 logging is no longer supported in WSO2 Identity Server.
...
In previous versions of the WSO2 Identity Server, the `authenticationendpoint` configurations, `accountrecoveryendpoint` configurations, and servlet configurations were done in the `web.xml` file. 

From WSO2 Identity Server 6.0.0 onwards the `authenticationendpoint` configurations, `accountrecoveryendpoint` configurations, and servlet configurations should be done in the `deployment.toml` file.
