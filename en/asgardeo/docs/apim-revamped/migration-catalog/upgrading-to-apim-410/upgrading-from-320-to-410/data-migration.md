...
From DB scripts included [here](../../../../../../api-manager/migration-resources/apim-4.1.0-resources/db-scripts/upgrading-from-320-to-410), run the script corresponding to your DB type on the `shared_db` (used as the registry database).
...
4. Run the below command to execute the pre-migration step that will validate your old data. If you're working with a distributed setup, remember to add the `-Dprofile=control-plane` property to the following commands as well.
    ...
    - Windows
      ```bash
      api-manager.bat -Dmigrate -DmigrateFromVersion=3.2.0 -DmigratedVersion=4.1.0 -DrunPreMigration
      ```
